...
import { Sun, Moon, Palette, Check, X, Type, ALargeSmall } from "lucide-react";
import {
                ...
                <div className="flex gap-1">
                  {(["color", "font"] as Panel[]).map((p) => (
                    <button key={p} onClick={() => setPanel(p)}
                      ...
                      {p === "color" ? <Palette size={12} /> : <Type size={12} />}
                      {p === "color" ? "Colors" : "Fonts"}
                    </button>
                  ))}
                </div>
                        ...
                        {themeList.map(({ key, emoji }) => {
                          ...
                          return (
                            <button key={key}
                              ...
                              {active && <Check size={13} style={{ color: t.accent }} />}
                            </button>
                          );
                        })}
                      </div>
